...
async def get_filled_games(
    ...
    try:
        query = select(game).where(
            (game.c.team_1 == team_id) | (game.c.team_2 == team_id)
        )
        result = await session.execute(query)
        games = []
        data = [dict(row) for row in result.mappings().all()]
        for row in data:
            if await is_team_filled(row["team_1"], session) and await is_team_filled(
                row["team_2"], session
            ):
                games.append(row["id"])
        return games
...
async def handle_add_team(
    team_create: TeamCreate, user: User, session: AsyncSession
) -> dict:
    try:
...
async def handle_update_team(
    team_id: int, positions: TeamCreate, user: User, session: AsyncSession
) -> dict:
    new_positions = {
        key: value for key, value in positions.dict().items() if value != 0
    }
    try:
        stmt = (
            update(team)
...
async def handle_join_team(
    team_id: int, position: int, user: User, session: AsyncSession
) -> dict:
    try:
        position_name = get_position_name(position)
        stmt = update(team).where(team.c.id == team_id).values({position_name: user.id})
        await session.execute(stmt)
        await session.commit()
        await update_filled_games(team_id, session)
        return {"status": "success", "data": None, "details": None}
